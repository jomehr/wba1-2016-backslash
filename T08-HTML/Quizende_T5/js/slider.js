...
var Slider = {
	...
	slide			: function() {
		...
		for(var i = 0; i < this.menuPoints.length; i++) {
			this.menuPoints[i].addEventListener("click", function (e) { Slider.menuPointClick(e); });
		}
		
		document.querySelector(this.nextElement).addEventListener("click", function() { Slider.next(); });
		document.querySelector(this.previousElement).addEventListener("click", function() { Slider.previous(); });
	...
	start			: function() {
		this.intervalFunc = setInterval(function(){Slider.next()}, this.interval);
	},
